...
def compare_copies(a, b, tol=0, exact=False, sort_df=True, col_type=True, verbose=False):
    import pandas as pd
    import numpy as np
    try:
        # list or tuple
        ...
        if isinstance(a, pd.DataFrame):
            try:
                ...
                return False
        # Series
        if isinstance(a, (int, float,  pd.Series)) \
            and isinstance(b, (int, float,  pd.Series)):
